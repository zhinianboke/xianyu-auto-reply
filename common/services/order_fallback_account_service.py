...
class OrderFallbackAccountService:
    ...
    async def list_configs(self, owner_id: int, is_admin: bool = False) -> List[Dict[str, Any]]:
        """列出兜底下单账号配置（按分类，含无分类那条），附带账号有效性。

        - 普通用户：仅返回本人配置；
        - 管理员：返回全部用户的配置，并附带所属用户名（owner_username）便于区分。
        """
        conditions = [OrderFallbackAccount.is_deleted.is_(False)]
        if not is_admin:
            conditions.append(OrderFallbackAccount.owner_id == owner_id)
        stmt = (
            select(OrderFallbackAccount)
            .where(*conditions)
            .order_by(
                OrderFallbackAccount.owner_id.asc(),
