...
功能：
1. 按分类查询/新建/修改/删除当前用户的兜底私信账号配置
   （每个用户每个分类一条；category_id=NULL 表示"无分类"全局兜底）
2. 供私信任务在商品下单账号发私信不可用时，按链回退取兜底账号：
...
class DmFallbackAccountService:
    """兜底私信账号配置服务（按分类配置）。"""
    ...
    async def list_configs(self, owner_id: int, is_admin: bool = False) -> List[Dict[str, Any]]:
        """列出当前用户已配置的兜底私信账号（按分类，含无分类那条），附带账号有效性。"""
        stmt = (
            select(DmFallbackAccount)
            .where(
                DmFallbackAccount.owner_id == owner_id,
                DmFallbackAccount.is_deleted.is_(False),
            )
            .order_by(
                DmFallbackAccount.category_id.is_(None).desc(),
                DmFallbackAccount.id.desc(),
            )
        )
        ...
        for record in records:
            data = self._to_dict(record, name_map)
    ...
    async def _get_record(
    ...
    ) -> Optional[DmFallbackAccount]:
        conditions = [DmFallbackAccount.owner_id == owner_id]
        if category_id is None:
            conditions.append(DmFallbackAccount.category_id.is_(None))
        else:
            conditions.append(DmFallbackAccount.category_id == category_id)
        if not include_deleted:
            conditions.append(DmFallbackAccount.is_deleted.is_(False))
        ...
        stmt = (
            ...
            .where(*conditions)
            .order_by(
                DmFallbackAccount.is_deleted.asc(),
                DmFallbackAccount.id.asc(),
            )
        )
    ...
    async def _admin_account_ids(self, category_id: Optional[int]) -> List[str]:
        """返回所有管理员在指定分类（或无分类）下配置的兜底私信账号ID（去重合并）。"""
        admin_ids_subq = select(User.id).where(User.role == UserRole.ADMIN)
        conditions = [
            DmFallbackAccount.owner_id.in_(admin_ids_subq),
            DmFallbackAccount.is_deleted.is_(False),
        ]
        if category_id is None:
            conditions.append(DmFallbackAccount.category_id.is_(None))
            ...
            conditions.append(DmFallbackAccount.category_id == category_id)
        stmt = select(DmFallbackAccount.account_ids).where(*conditions)
        result: List[str] = []
        seen: set[str] = set()
    ...
    def _to_dict(
        self,
        record: Optional[DmFallbackAccount],
