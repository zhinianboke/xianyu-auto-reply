...
class OrderService:
    ...
    async def fetch_xianyu_orders(
        ...
        cookie_unb = str(
            extract_account_user_id_from_cookie(account.cookie) or ""
        ).strip()
        if expected_unb and cookie_unb and cookie_unb != expected_unb:
            message = (
                f"账号 {account_id} 的 Cookie 实际身份 {cookie_unb} 与已记录身份 "
                f"{expected_unb} 不一致，已跳过订单同步，请重新登录该账号"
            )
            logger.warning(f"获取闲鱼订单: {message}")
            return {
                "total_fetched": 0,
                "new_inserted": 0,
                "updated": 0,
