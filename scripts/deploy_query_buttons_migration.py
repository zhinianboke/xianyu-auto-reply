...
# 「展示入口」配置（提货页底部的工具下载/API 文档等入口，与 DisplayLink 契约一致）
# 注意：url / content 里的接口地址请替换为你自己的
DISPLAY_LINKS = [
    {
        "name": "查询工具下载",
        "type": "link",
        "url": "https://pan.example.com/s/your-share-link",
        "note": "提取码：xxxx",
    },
    {
        "name": "余额API",
        ...
        "content": (
            "接口地址\n"
            "GET https://your-api.example.com/wallet/summary\n\n"
            "请求头\n"
            "Cookie: {cookie}\n\n"
            "使用说明\n"
            "1. 下载上方查询工具，按工具内指引填入 Cookie 即可随时查余额\n"
            "2. 也可以把本页接口信息和你的 Cookie 发给任意 AI，让它帮你查"
        ),
    },
]
...
def migrate_display_links(cursor, dry_run: bool) -> None:
    """给目标商品合并写入「展示入口」配置；已配置的行跳过。"""
    patch = json.dumps({"display_links": DISPLAY_LINKS}, ensure_ascii=False)
    cursor.execute(
        "SELECT item_id FROM xy_catalog_items "
        "WHERE item_id IN %s AND JSON_EXTRACT(COALESCE(metadata, '{}'), '$.display_links') IS NOT NULL",
        (TARGET_ITEM_IDS,),
    )
    configured = {row[0] for row in cursor.fetchall()}
    ...
    for item_id in TARGET_ITEM_IDS:
        if item_id in configured:
            print(f"  [跳过] item_id={item_id} 已存在 display_links 配置")
            continue
        print(f"  [写入] item_id={item_id} 合并 display_links 展示入口配置")
        if not dry_run:
            cursor.execute(
                "UPDATE xy_catalog_items "
                "SET metadata = JSON_MERGE_PATCH(COALESCE(metadata, '{}'), %s) "
                "WHERE item_id = %s "
                "AND JSON_EXTRACT(COALESCE(metadata, '{}'), '$.display_links') IS NULL",
                (patch, item_id),
            )
            if cursor.rowcount == 0:
                print(f"  [警告] item_id={item_id} 未更新任何行（商品不存在或已被并发写入）")
...
def main() -> None:
    ...
    try:
        with conn.cursor() as cursor:
            ...
            migrate_display_links(cursor, args.dry_run)
