 ...
 * 2. URL 带 orderNo/button 参数时（从提货页跳转过来）自动填入订单号，查询由买家手动点击执行
...
export function QueryPage() {
  ...
  // 查询页顶部提示文案（来自商品配置 metadata_json.page_hint），空串不显示
  ...
  const loadButtons = async (no: string): Promise<PublicQueryButton[] | null> => {
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        setPageHint(res.data.page_hint || '')
        // 手动 Cookie 查询默认选中第一个含 {cookie} 变量的按钮（用其在完整配置中的原始下标）
        const cookieBtn = list.find((b) => b.uses_cookie)
        if (cookieBtn) setCookieButtonIndex(cookieBtn.index ?? 0)
  ...
  // URL 参数自动填入订单号并加载按钮列表，但不自动执行——查询需买家手动点击触发
  useEffect(() => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-600 via-blue-500 to-indigo-600 flex items-center justify-center p-4">
      <div className="w-full max-w-sm bg-white dark:bg-slate-800 rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="p-6 flex flex-col gap-5">
          {/* 查询页提示文案：来自商品配置 metadata_json.page_hint，空则不显示 */}
          {pageHint && (
            <div className="flex items-start gap-2 rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 px-4 py-3">
              ...
              <p className="text-xs text-amber-700 dark:text-amber-300 leading-relaxed whitespace-pre-line">
                {pageHint}
              </p>
            </div>
          )}
          ...
          {buttonsLoaded && buttons.length > 0 && (
            <div className="flex flex-col gap-2">
              ...
              {buttons.map((btn, idx) => {
                // 停用按钮不下发，index 为其在完整配置中的原始下标，执行时原样回传
                const btnIndex = btn.index ?? idx
                return (
                <button
                  key={btnIndex}
                  type="button"
                  onClick={() => executeButton(orderNo.trim(), btnIndex)}
                ...
                <div className="p-4 flex flex-col gap-3">
                  {buttons.length > 1 && (
                    <div>
                      ...
                      >
                        {buttons.map((btn, idx) => (
                          <option key={btn.index ?? idx} value={btn.index ?? idx}>{btn.name}</option>
                        ))}
                      </select>
                    </div>
                  )}
