...
// ==================== 展示入口（提货页底部工具区，存商品 metadata_json.display_links 数组） ====================

interface DisplayLinkBase {
  /** 按钮名称 */
  name: string
}

/** 外链入口：新窗口打开 url，note 显示在按钮右侧 */
export interface DisplayLinkLink extends DisplayLinkBase {
  type: 'link'
  /** 链接地址，必须 http(s) */
  url: string
  /** 备注（如「提取码：xxxx」），可选 */
...
/** 文本入口：点击弹窗展示 content，{cookie} 由提货页替换为发货内容中的 Cookie */
export interface DisplayLinkText extends DisplayLinkBase {
  type: 'text'
  /** 弹窗标题 */
  title: string
  /** 弹窗内容（多行文本，支持 {cookie} 占位符） */
  content: string
}
...
export const getItemQueryButtons = (
  cookieId: string,
  itemId: string,
): Promise<ApiResponse<{ buttons: QueryButton[]; page_hint?: string }>> => {
...
export const saveItemQueryButtons = (
  ...
  return put(`${ITEM_PREFIX}/${cookieId}/${itemId}/query-buttons`, { buttons, page_hint: pageHint ?? '' })
}

/** 获取商品展示入口配置（提货页底部工具区） */
export const getItemDisplayLinks = (
  cookieId: string,
  itemId: string,
): Promise<ApiResponse<{ links: DisplayLink[] }>> => {
  return get(`${ITEM_PREFIX}/${cookieId}/${itemId}/display-links`)
}

/** 保存商品展示入口配置（整体覆盖） */
export const saveItemDisplayLinks = (
  cookieId: string,
  itemId: string,
  links: DisplayLink[],
): Promise<ApiResponse> => {
...
export interface PublicQueryButton {
  ...
  uses_cookie?: boolean
  /** 在商品完整配置数组中的原始下标（停用按钮不下发，下标不连续），执行时原样回传 */
  index?: number
}
