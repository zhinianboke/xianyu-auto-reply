...
const DEFAULT_REPLY_FALLBACK: ItemDefaultReplyConfig = {
  item_id: '',
  reply_content: '',
  reply_image: '',
  enabled: false,
...
/**
 * 获取商品默认回复配置。
 * 后端: GET /api/v1/items/{cookie_id}/{item_id}/default-reply → data: ItemDefaultReplyConfig
 * 未配置时后端也返回 success=true + 默认值，此处再兜底一层。
 */
export async function getItemDefaultReply(
  cookieId: string,
  itemId: string,
): Promise<ItemDefaultReplyConfig> {
  const client = await getApiClient();
  const { data, error } = (await (client.GET as any)(
    `${PREFIX}/${encodeURIComponent(cookieId)}/${encodeURIComponent(itemId)}/default-reply`,
  )) as { data?: unknown; error?: unknown };
  if (error) throw await extractError(error);
  const res = (data ?? {}) as Record<string, unknown>;
  if (res.success === false) {
    throw new Error(
      typeof res.message === 'string' && res.message ? res.message : '获取默认回复失败',
    );
  }
  const inner =
    res.data && typeof res.data === 'object' ? (res.data as Record<string, unknown>) : res;
  return {
    ...DEFAULT_REPLY_FALLBACK,
    ...(inner as Partial<ItemDefaultReplyConfig>),
    item_id: typeof inner.item_id === 'string' ? inner.item_id : itemId,
  };
}
...
export async function saveItemDefaultReply(
  cookieId: string,
  itemId: string,
  config: Omit<ItemDefaultReplyConfig, 'item_id'>,
): Promise<string> {
  const client = await getApiClient();
  const { data, error } = (await (client.PUT as any)(
    `${PREFIX}/${encodeURIComponent(cookieId)}/${encodeURIComponent(itemId)}/default-reply`,
    {
      body: {
        ...
        reply_image: config.reply_image,
  ...
  if (error) throw await extractError(error);
  return assertFlagOk(data, '商品默认回复已保存');
}

// ---------------------------------------------------------------------------
// 商品AI提示词（对齐 web src/api/items.ts 的 getItemAiPrompt/saveItemAiPrompt）
// ---------------------------------------------------------------------------
...
/**
 * 获取商品AI提示词配置。
 * 后端: GET /api/v1/items/{cookie_id}/{item_id}/ai-prompt → data: { item_id, ai_prompt }
 */
export async function getItemAiPrompt(
  cookieId: string,
  itemId: string,
): Promise<ItemAiPromptConfig> {
  const client = await getApiClient();
  const { data, error } = (await (client.GET as any)(
    `${PREFIX}/${encodeURIComponent(cookieId)}/${encodeURIComponent(itemId)}/ai-prompt`,
  )) as { data?: unknown; error?: unknown };
  if (error) throw await extractError(error);
  const res = (data ?? {}) as Record<string, unknown>;
  if (res.success === false) {
    throw new Error(
      typeof res.message === 'string' && res.message ? res.message : '获取AI提示词失败',
    );
  }
  const inner =
    res.data && typeof res.data === 'object' ? (res.data as Record<string, unknown>) : res;
  return {
    item_id: typeof inner.item_id === 'string' ? inner.item_id : itemId,
    ai_prompt: typeof inner.ai_prompt === 'string' ? inner.ai_prompt : '',
  };
}

/**
 * 保存商品AI提示词配置。
 * 后端: PUT /api/v1/items/{cookie_id}/{item_id}/ai-prompt，body: { ai_prompt }
 */
export async function saveItemAiPrompt(
  cookieId: string,
  itemId: string,
  aiPrompt: string,
): Promise<string> {
  ...
  if (error) throw await extractError(error);
  return assertFlagOk(data, '商品AI提示词已保存');
}
