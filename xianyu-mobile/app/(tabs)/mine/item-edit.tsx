...
// 查询配置编辑态 ⇄ QueryButton 的序列化/反序列化已提取到 @/lib/query-button-draft，
// 供本页与素材编辑页(material-edit)复用。
...
export default function ItemEditScreen() {
  ...
  const loadQueryButtons = useCallback(async () => {
    ...
    try {
      const cfg = await getItemQueryButtons(cookieId, itemId);
      setQueryDrafts(cfg.buttons.map(draftFromButton));
      setQueryPageHint(cfg.pageHint);
      setQueryLoaded(true);
    } catch (e) {
  ...
  async function handleSaveQueryButtons() {
    ...
    setQuerySaving(true);
    try {
      const msg = await saveItemQueryButtons(cookieId, itemId, buttons, queryPageHint);
