...
export default function CardsScreen() {
  ...
  const loadMore = useCallback(() => {
    // 首次加载未完成(total=0)或正在加载/刷新时不追加，避免 onEndReached 与 reset 请求
    // 交错导致 setCards 在空/满之间反复跳、FlatList 重新布局抖动
    if (loading || loadingMore || refreshing) return;
    if (total === 0) return;
    if (totalPages > 0 && page >= totalPages) return;
    return fetchPage(page + 1, 'append');
  }, [loading, loadingMore, refreshing, total, totalPages, page, fetchPage]);
          ...
          return (
            ...
            >
              ...
              >
                ...
                {/* 具体内容预览：data 显示剩余条数、text 显示内容、api 显示地址、image 显示图片数 */}
                {item.type === 'data' && item.data_content ? (
                  <Text style={[styles.cardDesc, { color: c.textSecondary }]} numberOfLines={1}>
                    剩余 {countDataLines(item.data_content)} 条库存
                  </Text>
                ) : item.type === 'text' && item.text_content ? (
                  <Text style={[styles.cardDesc, { color: c.textSecondary }]} numberOfLines={1}>
                    内容：{item.text_content.slice(0, 40)}
                  </Text>
                ) : item.type === 'api' && item.api_config?.url ? (
                  <Text style={[styles.cardDesc, { color: c.textSecondary }]} numberOfLines={1}>
                    接口：{item.api_config.url}
                  </Text>
                ) : item.type === 'image' && item.image_urls && item.image_urls.length > 0 ? (
                  <Text style={[styles.cardDesc, { color: c.textSecondary }]} numberOfLines={1}>
