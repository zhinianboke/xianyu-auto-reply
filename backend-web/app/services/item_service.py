...
class ItemService:
    ...
    async def save_fetched_items(
        ...
        account: XYAccount,
        items: list[dict],
        existing_map: dict[str, XYCatalogItem] | None = None,
    ) -> int:
        """保存抓取到的商品数据到本地库"""
        try:
            ...
            if existing_map is None:
                item_ids = list(dict.fromkeys(item_id for item_id, _ in valid_items))
                existing_map = await self._get_existing_item_map(account, item_ids)
