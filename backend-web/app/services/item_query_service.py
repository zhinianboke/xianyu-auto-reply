...
class ItemQueryService:
    ...
    async def get_page_hint_for_item(self, owner_id: int, item_id: str) -> str:
        """读取商品配置的查询页提示文案（metadata_json.page_hint），无配置返回空串。"""
        if not item_id:
            return ""
        result = await self.session.execute(
            select(XYCatalogItem).where(
        ...
        if not item:
            return ""
        return (item.metadata_json or {}).get("page_hint") or ""

    async def get_buttons_public(self, order_no: str) -> tuple[bool, str, dict]:
        """按订单号返回买家可见的按钮列表（只含按钮名）与 uses_cookie 标记。

        Returns:
            (success, message, data)；data 为 ``{"buttons": [{"name": ...}], "uses_cookie": bool, "page_hint": str}``，
            失败时 data 为 ``{"buttons": [], "uses_cookie": False, "page_hint": ""}``。
        """
        empty = {"buttons": [], "uses_cookie": False, "page_hint": ""}
        ...
        # 只下发启用中的按钮（配置里 enabled=false 的按钮对买家隐藏）；
        # index 保留其在完整配置数组中的原始下标，供执行接口按原下标定位
        enabled_buttons = [
            (idx, button) for idx, button in enumerate(buttons)
            if button.get("enabled", True) is not False
        ]
        return True, "查询成功", {
            # 每个按钮带 uses_cookie 标记：前端手动 Cookie 查询需要定位到含 {cookie} 变量的按钮
            "buttons": [
                {"name": button.get("name") or "查询", "uses_cookie": self._button_uses_cookie(button), "index": idx}
                for idx, button in enabled_buttons
            ],
            "uses_cookie": any(self._button_uses_cookie(button) for _, button in enabled_buttons),
            "page_hint": await self.get_page_hint_for_item(order.owner_id, order.item_id or ""),
        }
    ...
    async def execute(
        ...
        if button.get("enabled", True) is False:
            return False, "该查询功能已停用，请联系卖家", {"results": []}
