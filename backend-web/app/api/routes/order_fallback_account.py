...
2. 新建/修改某个分类的兜底下单账号配置（每用户每分类一条；无分类仅一条）
3. 删除某个分类的兜底下单账号配置（软删除）

说明：当定时下单/私信任务发现监控任务自身无可用下单账号时，按 5 层链回退：
...
class OrderFallbackAccountSaveRequest(BaseModel):
    ...
    account_ids: List[str] = Field(default_factory=list, description="兜底下单账号ID列表（可多选，可为空）")
...
async def list_order_fallback_accounts(
    current_user: User = Depends(get_current_active_user),
    session: AsyncSession = Depends(get_db_session),
) -> Dict[str, Any]:
    """列出兜底下单账号配置（按分类）。管理员可查看全部用户的配置。"""
...
async def upsert_order_fallback_accounts(
...
async def delete_order_fallback_accounts(
    ...
    """删除某个分类的兜底下单账号配置（软删除）。管理员可删除其他用户的配置。"""
    svc = OrderFallbackAccountService(session)
    try:
