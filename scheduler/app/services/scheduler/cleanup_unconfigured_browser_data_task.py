...
功能：
1. 每3小时执行一次
2. 查询数据库中 username 或 login_password 为空（未配置账号密码）的账号
3. 删除对应的 browser_data/user_{account_id} 目录
4. 仅按"是否配置账号密码"判断，不涉及账号禁用逻辑（禁用清理由其它任务处理）
"""
import shutil
...
class CleanupUnconfiguredBrowserDataTaskService:
    ...
    async def execute(self):
        ...
        try:
            ...
            # 2. 查询所有未配置账号密码的账号
            unconfigured_accounts = await self._get_unconfigured_accounts()

            logger.info(
                f"【{self.task_name}】查询到 {len(unconfigured_accounts)} 个"
                f"未配置账号密码的账号"
            )
            ...
            for account_id in unconfigured_accounts:
                try:
    ...
    async def _get_unconfigured_accounts(self) -> list[str]:
        """
        获取所有未配置账号密码的账号ID列表

        判断条件：username 为空/NULL 或 login_password 为空/NULL
        即账号未同时配置登录用户名与登录密码
        ...
        try:
            async with async_session_maker() as session:
                # 查询 username 或 login_password 未配置（NULL 或空字符串）的账号
                ...
                logger.debug(
                    f"【{self.task_name}】查询到 {len(account_ids)} 个未配置账号密码的账号"
                )
                return account_ids

        except Exception as e:
            logger.error(f"【{self.task_name}】查询未配置账号密码的账号失败: {e}")
            return []
...
# 创建全局实例
cleanup_unconfigured_browser_data_task_service = CleanupUnconfiguredBrowserDataTaskService()
